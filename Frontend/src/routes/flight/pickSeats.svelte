<script lang="ts">
    ...
    const miniOnMount = async() => {
        ...
        if(retourFlight){
            getData = await get(`http://localhost:3001/api/v1/flight/seats/${$FlightStore.retourFlight}`)
            console.log(getData)
        }else{
            getData = await get(`http://localhost:3001/api/v1/flight/seats/${$FlightStore.departureFlight}`)
            console.log(getData)
        }
